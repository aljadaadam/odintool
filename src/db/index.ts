...
function migrate(db: any) {
  ...
  try {
    db.exec(
      'CREATE UNIQUE INDEX IF NOT EXISTS idx_product_variants_externalKey ON product_variants(externalKey)'
    );
  } catch {
    // ignore
  }
  ...
  try {
    db.exec(
      "UPDATE product_variants SET creditCost = COALESCE(creditCost, 0) WHERE creditCost IS NULL"
    );
    db.exec(
      "UPDATE product_variants SET creditCost = CASE WHEN creditCost = 0 AND creditText <> '' THEN CAST(creditText AS REAL) ELSE creditCost END"
    );
  } catch {
    // ignore
  }
  ...
  try {
    db.exec(
      "UPDATE licenses SET durationYears = COALESCE(durationYears, 1) WHERE durationYears IS NULL"
    );
    db.exec("UPDATE licenses SET activatedAt = COALESCE(activatedAt, createdAt) WHERE activatedAt IS NULL");
    db.exec(
      "UPDATE licenses SET expiresAt = COALESCE(expiresAt, datetime(activatedAt, '+1 year')) WHERE expiresAt IS NULL"
    );
  } catch {
    // ignore
  }
...
function seedInventory(db: any) {
  ...
  upsertVariant.run(
    'muslim-odin-lifetime-mtkpro',
    'muslim-odin',
    'Muslim Odin life time activation + MTK PRO',
    '18153',
    163382,
    99,
    'manual',
    requiredUserAndPhone,
    'create_activation_job',
    'activation',
    '22.000',
    '1-24 Hours',
    ''
